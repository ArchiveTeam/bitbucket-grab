...
LUA_SHA1 = get_hash(os.path.join(CWD, 'bitbucket.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'bitbucket-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('bitbucket-item: %(item_name)s'),
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='bitbucket'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
